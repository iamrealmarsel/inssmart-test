export default class Tabs {
  ...
  public init() {
    ...
    this.tabElements[0].classList.add('active');
  ...
  private handleTabClick(i: number) {
    this.contentElements.forEach((elem) => elem.classList.remove('active'));
    this.contentElements[i].classList.add('active');

    this.tabElements.forEach((elem) => elem.classList.remove('active'));
    this.tabElements[i].classList.add('active');
  }
